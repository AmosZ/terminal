...
class Window(Container, gtk.Window):
    ...
    def tab_new(self, widget=None, debugtab=False, _param1=None, _param2=None):
        ...
        if not self.is_child_notebook():
            ...
            pre_page = self.get_child().get_current_page()
        else:
            notebook = self.get_child()
            pre_page = self.get_child().get_current_page()
        self.get_child().newtab(debugtab, cwd=cwd)
        
        #Move tab next to current tab#
        current_page = self.get_child().get_current_page();
        child = self.get_child().get_nth_page(current_page)
        notebook.reorder_child(child,pre_page+1);
